...
class CI_Upload {
	...
	public function do_upload($field = 'userfile', $num = 0 )
	{
		if(is_array( $_FILES[$field]['tmp_name'] ) ) {//배열인경우 첫번째꺼처리 @2012-05-28
			...
			if ( ! is_uploaded_file($_FILES[$field]['tmp_name'][$num]))
			{
				$error = ( ! isset($_FILES[$field]['error'][$num])) ? 4 : $_FILES[$field]['error'][$num];
			...
			$this->file_temp = $_FILES[$field]['tmp_name'][$num];
			$this->file_size = $_FILES[$field]['size'][$num];
			$this->_file_mime_type($_FILES[$field], $num);
			$this->file_type = preg_replace("/^(.+?);.*$/", "\\1", $this->file_type);
			$this->file_type = strtolower(trim(stripslashes($this->file_type), '"'));
			$this->file_name = $this->_prep_filename($_FILES[$field]['name'][$num]);
	...
	public function is_allowed_filetype($ignore_mime = FALSE)
	{
		...
		if ( in_array($ext, $this->allowed_types) && !in_array($ext, $image_types))
		{
			return TRUE;
		}
	...
	protected function _file_mime_type($file, $num = 0 )
	{
		if( is_array($file['tmp_name']) ){
			...
			if ( (float) substr(phpversion(), 0, 3) >= 5.3 && function_exists('finfo_file'))
			{
				...
				{
					$file_type = $finfo->file($file['tmp_name'][$num]);
			...
			{
				$output = array();
				@exec('file --brief --mime-type ' . escapeshellarg($file['tmp_path'][$num]), $output, $return_code);
