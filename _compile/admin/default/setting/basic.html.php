<?php /* Template_ 2.2.6 2014/11/03 11:38:03 /www/nsone_firstmall_kr/admin/skin/default/setting/basic.html 000022507 */ 
		...
		<div class="slc-body">
			...
			<tr>
				...
				<td class="its-td">
                    <table>
                        <tr>
                            <td>정식 : http:// <input type="text" name="domain" value="<?php echo $TPL_VAR["config_system"]["domain"]?>" size="30" class="line" /></td>
                            <td>&nbsp;<span class="desc">브라우저로 접속이 가능한 연결된 도메인을 입력하세요!</td>
                        </tr>
                        <tr>
                            <td>임시 : http:// <b><?php echo $TPL_VAR["config_system"]["subDomain"]?></b></td>
                            <td>&nbsp;
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="shopName" value="<?php echo $TPL_VAR["shopName"]?>" size="40" class="line" /> <span class="desc">예) 슬이나라</span>
				</td>
			</tr>
			<tr>
...
<?php }}?>
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="shopTitleTag" value="<?php echo $TPL_VAR["shopTitleTag"]?>" size="60" title="브라우저 상단에 표기되는 타이틀을 입력하세요" class="line" />
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="shopGoodsTitleTag" value="<?php echo $TPL_VAR["shopGoodsTitleTag"]?>" size="60" title="브라우저 상단에 표기되는 타이틀을 입력하세요" class="line" />
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="shopCategoryTitleTag" value="<?php echo $TPL_VAR["shopCategoryTitleTag"]?>" size="60" title="브라우저 상단에 표기되는 타이틀을 입력하세요" class="line" />
...
<?php if($TPL_VAR["config_system"]["favicon"]){?>
					<span>
					<img src="<?php echo $TPL_VAR["config_system"]["favicon"]?>" alt="파비콘" align="absmiddle" onerror="this.style.display='none'" />
...
<?php }?>
			...
			<tr>
				...
				<td class="its-td">
					...
					적립금 <input type="text" name="default_reserve_bookmark" style="text-align:right" size="5" class="line onlynumber" value="<?php echo $TPL_VAR["reserve"]["default_reserve_bookmark"]?>" />원. 지급 적립금의 유효기간은 <select name="book_reserve_select">
						<option value="">제한하지 않음</option>
						<option value="year" <?php if($TPL_VAR["reserve"]["book_reserve_select"]=='year'){?>selected<?php }?>>제한 - 12월31일</option>
						<option value="direct" <?php if($TPL_VAR["reserve"]["book_reserve_select"]=='direct'){?>selected<?php }?>>제한 - 직접입력</option>
					</select>
					<span name="reserve_y" class="hide">→ 지급연도 + <input type="text" name="book_reserve_year" class="line onlynumber" style="text-align:right" size="3" maxlength="3" value="<?php echo $TPL_VAR["reserve"]["book_reserve_year"]?>" min="0" max="9" />년 12월 31일</span>
					<span name="reserve_d" class="hide">→ <input type="text" name="book_reserve_direct" class="line onlynumber" style="text-align:right" size="3" value="<?php echo $TPL_VAR["reserve"]["book_reserve_direct"]?>" />개월</span>                          <br/>
					...
					포인트 <input type="text" name="default_point_bookmark" style="text-align:right" size="5" class="line onlynumber" value="<?php echo $TPL_VAR["reserve"]["default_point_bookmark"]?>" />p. 지급 포인트의 유효기간은 <select name="book_point_select" <?php if(!$TPL_VAR["isplusfreenot"]||!$TPL_VAR["isplusfreenot"]["ispoint"]){?> readonly="readonly" disabled='disabled'  class="gray readonly"  <?php }?>  >
						<option value="">제한하지 않음</option>
						<option value="year" <?php if($TPL_VAR["reserve"]["book_point_select"]=='year'){?>selected<?php }?>>제한 - 12월31일</option>
						<option value="direct" <?php if($TPL_VAR["reserve"]["book_point_select"]=='direct'){?>selected<?php }?>>제한 - 직접입력</option>
					</select>
					<span name="point_y" class="hide">→ 지급연도 + <input type="text" name="book_point_year" class="line onlynumber" min="0" max="9" style="text-align:right" size="3" maxlength="3" value="<?php echo $TPL_VAR["reserve"]["book_point_year"]?>" />년 12월 31일</span>
			...
			<tr>
				...
				<td class="its-td">
					...
					<label><input type="radio" name="metaTagUse" value="1" <?php if($TPL_VAR["metaTagUse"]){?>checked<?php }?> />검색엔진 정보수집 허용</label>
					<label><input type="radio" name="metaTagUse" value="0" <?php if(!$TPL_VAR["metaTagUse"]){?>checked<?php }?> />검색엔진 정보수집 미허용 (구글, 다음 등 각종 포털)</label>
					</td>
			</tr>
			<tr>
				...
				<td class="its-td">
					...
					<tr>
						<td>
							<textarea name="metaTagDescription" style="width:330px; height:70px;" rows="4" class="line" title="검색엔진에서 수집할 사이트의 설명을 입력하세요" ><?php echo $TPL_VAR["metaTagDescription"]?></textarea>
						</td>
						<td></td>
						<td>
							<textarea name="metaTagKeyword" style="width:330px; height:70px;" class="line" title="검색엔진에서 수집할 사이트의 키워드(콤마구분)을 입력하세요.<?php echo chr( 10)?>예) 여성의류, 캐주얼 패션, 청소년 의류"><?php echo $TPL_VAR["metaTagKeyword"]?></textarea>
						</td>
					</tr>
					</table>
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="companyName" value="<?php echo $TPL_VAR["companyName"]?>" class="line" />
				</td>
				<th class="its-th">업태/종목</th>
				<td class="its-td">
					<input type="text" name="businessConditions" value="<?php echo $TPL_VAR["businessConditions"]?>" class="line" title="예) 도소매" /> / <input type="text" name="businessLine" value="<?php echo $TPL_VAR["businessLine"]?>" title="예) 전자상거래" />
				</td>
			</tr>
			<tr>
				...
				<td class="its-td">
					<input type="text" name="companyEmail[]" value="<?php echo $TPL_VAR["companyEmail"][ 0]?>" size="10" class="line" /> @
					<input type="text" name="companyEmail[]" value="<?php echo $TPL_VAR["companyEmail"][ 1]?>" size="10" class="emailListInput line" />
...
<?php }}?>
			...
			<tr>
				...
				<td class="its-td">
					<input type="text" name="ceo" value="<?php echo $TPL_VAR["ceo"]?>" class="line" />
				</td>
...
<?php }}?>
					...
					<script type="text/javascript">
						$("select[name='companyPhone[]']").val('<?php echo $TPL_VAR["companyPhone"][ 0]?>');
					</script>
					-->


					<input type="text" name="companyPhone[]" value="<?php echo $TPL_VAR["companyPhone"][ 0]?>" size="5" class="line" />
					- <input type="text" name="companyPhone[]" value="<?php echo $TPL_VAR["companyPhone"][ 1]?>" size="5" class="line" />
					- <input type="text" name="companyPhone[]" value="<?php echo $TPL_VAR["companyPhone"][ 2]?>" size="5" class="line" />
				</td>
			</tr>
			<th class="its-th">개인정보관리 책임자</th>
				<td class="its-td">
					<input type="text" name="member_info_manager" value="<?php echo $TPL_VAR["member_info_manager"]?>" class="line" />
				</td>
			...
			<tr>
				...
				<td class="its-td" colspan="3">
					<input type="text" name="mailsellingLicense" value="<?php echo $TPL_VAR["mailsellingLicense"]?>" class="line" /> <span class="desc">예) 강남 제 000호 (또는 간이과세자로서 통신판매신고번호가 없는 경우 '간이과세자'로 입력하세요)</span>
				</td>
			</tr>
			<tr>
				<th class="its-th">사업장 주소</th>
				<td class="its-td" colspan="3">
					<input type="text" name="companyZipcode[]" value="<?php echo $TPL_VAR["companyZipcode"][ 0]?>" size="5" class="line" /> -
					<input type="text" name="companyZipcode[]" value="<?php echo $TPL_VAR["companyZipcode"][ 1]?>" size="5" class="line" />
					<span class="btn small"><input type="button" id="companyZipcodeButton" value="우편번호" /></span><br>
					<input type="text" name="companyAddress_type" value="<?php echo $TPL_VAR["companyAddress_type"]?>" class="hide" />
					<span <?php if($TPL_VAR["companyAddress_type"]!="street"){?>style="font-weight:bold;"<?php }?>>(지번) </span><input type="text" name="companyAddress" value="<?php echo $TPL_VAR["companyAddress"]?>" size="98" class="line" /><br>
					<span <?php if($TPL_VAR["companyAddress_type"]=="street"){?>style="font-weight:bold;"<?php }?>>(도로명) </span><input type="text" name="companyAddress_street" value="<?php echo $TPL_VAR["companyAddress_street"]?>" size="96" class="line" /><br>
					(상세주소) <input type="text" name="companyAddressDetail" value="<?php echo $TPL_VAR["companyAddressDetail"]?>" size="100" class="line" />
				</td>
			</tr>
			<tr>
				<th class="its-th">네이버 맵 키 <span class="helpicon" title="네이버에서 키를 발급 받으세요."></span></th>
				<td class="its-td" colspan="3">					
					<input type="text" name="mapKey" value="<?php echo $TPL_VAR["mapKey"]?>" size="40" class="line" /><a href="http://developer.naver.com/wiki/pages/MapAPI" target="_blank">키발급받기</a>
...
<?php if($TPL_VAR["config_system"]["mall_auth_yn"]!='y'){?>
			<div class="pd10">
				...
				&nbsp;&nbsp;&nbsp;<a href='http://firstmall.kr/myshop/spec/manager_information.php?num=<?php echo $TPL_VAR["config_system"]["shopSno"]?>' target='_blank' style="color:#0251d4;font-weight:Bold;text-decoration:underline;">[My가비아 > 쇼핑몰관리 > 기본사양관리 > 쇼핑몰관리자 정보관리]</a><span style="color:#666">에서 인증절차를 진행 해 주시기 바랍니다.</span>
			</div>
<?php }?>
