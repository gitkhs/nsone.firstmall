<?php /* Template_ 2.2.6 2014/11/03 11:38:03 /www/nsone_firstmall_kr/admin/skin/default/category/catalog.html 000021167 */ 
...
$(function () {
	...
	$("#setGroup").live("click",function(){
		openDialog("접속제한 <span class='desc'>카테고리를 접속할 회원그룹을 설정합니다.</span>", "setGroupsPopup", {"width":"500","height":150});
	});
...
function groupsMsg(){
	var str='';
	var tag='';
	$("#groupsMsg").html("이 카테고리 접속권한은 제한 없습니다.");
	...
	if(str){
		var msg = "이 카테고리 접속권한은 " + str.substr(0,str.length-1) + " 회원에게 있습니다." + tag;		
		$("#groupsMsg").html(msg);
...
function viewCategoryInfo(opts){
	...
	if(categoryCode){
		if(mode=='info'){
			...
			$("#ifrmCategorySetting").attr('src','/admin/category/ifrm_category_info?categoryCode='+categoryCode);
		}
		if(mode=='design'){
			$("#categorySettingContainer").append('<iframe id="ifrmCategorySetting" name="ifrmCategorySetting" style="width:100%; height:100px;" frameborder="0"></iframe>');
			$("#ifrmCategorySetting").attr('src','/admin/category/ifrm_category_design?categoryCode='+categoryCode);
		}
...
function categorySettingFormSubmit(){
	...
	if($("#ifrmCategorySetting").length){
		...
		openDialogAlert("카테고리를 선택해주세요..",400,140);
	}
...
<!-- 서브메뉴 바디 : 시작-->
	...
	<div class="relative" style="margin-top:1px;">
		<div class="absolute" style="top:0px; left:0px; width:310px;">
			<table width="100%" class="info-table-style">
			<col />
			<tr>
				<th class="its-th" id="mmenu">
				카테고리
				<span class="btn small cyanblue"><input type="button" id="add_folder" value="+" style="display:block; float:left;"/></span>
				<span class="btn small red"><input type="button" id="remove_button" value="-" style="display:block; float:left;"/></span>
				
				<input type="button" id="remove" value="-" style="display:none; float:left;"/>
				<span style="float:right; padding-right:5px;">카테고리영역 디자인  <span class="helpicon" title="<?php echo $TPL_VAR["category_design_ex"]?>"></span></span>
				</th>		
			</tr>	
			<tr>
			...
			<table class="help-table-style" width="100%" style="margin-top:2px">
			<tr><td colspan="2" style="height:30px; background:#7498b3; color:#ffffff; padding-left:15px;"> <b>카테고리 기능 안내</b></td></tr>
			<tr>
				<td class="help-th" width="76px"><span class="btn small cyanblue"><input type="button" value="+" style="display:block; "/></span></td>
				<td class="help-td">카테고리를 생성합니다.</td>
			</tr>
			<tr>
				<td class="help-th"><span class="btn small red"><input type="button" value="-" style="display:block; "/></span></td>
			...
			<tr>
				<td class="help-th"><img src="/admin/skin/default/images/common/btn_makeup.gif"></td>
				<td class="help-td">해당 카테고리 페이지를 꾸밀 수 있습니다.</td>
			</tr>
			<tr>
		...
		<div style="padding-left:311px;">
			...
			<col />	
			<tr>
			<th class="its-th" style="height:20px">카테고리 정보</th>
			</tr>
			<tbody>
			<tr>				
				<td class="its-td">
					<span class="btn small gray"><input type="button" value="+"/></span> 버튼을 클릭하여 카테고리를 생성할 수 있습니다.
				</td>
			</tr>
			</tbody>
			</table>
...
$("#tree")
		...
		"contextmenu" : {
			"items" : {
				"test" : {
					"label" : "꾸미기",
					"action" : function(obj){					
						var category = $(obj).find('ins').attr('category');
						viewCategoryInfo({'mode':'design','categoryCode':category})
					}
				}
			}
	...
	.bind("load_node.jstree", function (node) {
		...
		$("li[rel!='drive']",node.target).each(function(){
			...
			{
				var pannel = $("<span class='categoryItemPannerl'><img src='/admin/skin/default/images/common/btn_makeup.gif' align='absmiddle' alt='꾸미기' class='hand' onclick=\"viewCategoryInfo({'mode':'design','categoryCode':'"+category+"'});\" /></span>");
				$(this).append(pannel);
			}
		});
		
		if($(".categoryBatchSettingBtn").length==0)
		{
			$("li[rel='drive']",node.target).eq(0).append(
					$("<span class='categoryBatchSettingBtn btn small red'><input type='button' value='한꺼번에 꾸미기' /></span>").click(function(){
						document.location.href='../category/batch_design_setting';
					})
			);
		}
