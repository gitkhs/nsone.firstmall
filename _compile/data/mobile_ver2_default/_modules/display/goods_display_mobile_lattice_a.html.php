<?php /* Template_ 2.2.6 2015/03/11 14:46:14 /www/nsone_firstmall_kr/data/skin/mobile_ver2_default/_modules/display/goods_display_mobile_lattice_a.html 000012031 */  $this->include_("snsLikeButton");
...
<style>
#<?php echo $TPL_VAR["display_key"]?> .goods_list {margin:4px; text-align:center; font-size:0px; *zoom:1; }
#<?php echo $TPL_VAR["display_key"]?> .goods_list:after {clear:both; display:block; content:"";}
#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item {display:inline-block; padding:0px 4px; vertical-align:top; overflow:hidden; width:<?php echo  100/$TPL_VAR["count_w"]?>%; box-sizing:border-box; font-size:12px;}
#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item_small {width:<?php echo  100/$TPL_VAR["count_w"]/ 2?>%; box-sizing:border-box; padding:0 4px;}
#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item>a {display:block; padding-top:5px; padding-bottom:5px;}
#<?php echo $TPL_VAR["display_key"]?> .goods_list span.gli_image {width:100%;}
#<?php echo $TPL_VAR["display_key"]?> .goods_list span.gli_image img {width:100%;}
#<?php echo $TPL_VAR["display_key"]?> .goods_list ol.gli_contents {padding-left:10px; padding-top:10px; text-align:<?php echo $TPL_VAR["text_align"]?>;}
...
<script>
var gl_item_resize_<?php echo $TPL_VAR["display_key"]?> = function(){
	if($(window).width()>=500 && $("#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item").length>=<?php echo $TPL_VAR["count_w"]* 2?>){
		$("#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item").not('.gl_item_small').addClass('gl_item_small');
	}else{
		$("#<?php echo $TPL_VAR["display_key"]?> .goods_list li.gl_item_small").removeClass('gl_item_small');
	}
};
var gl_item_resize_timer_<?php echo $TPL_VAR["display_key"]?> = null;

$(function(){
	$(window).bind('resize',function(){
		gl_item_resize_<?php echo $TPL_VAR["display_key"]?>();
	}).trigger('resize');

	$(".goodsDisplayImageWrap>img").load(function(){
		gl_item_resize_timer_<?php echo $TPL_VAR["display_key"]?> = setInterval(function(){
			clearInterval(gl_item_resize_timer_<?php echo $TPL_VAR["display_key"]?>);
			gl_item_resize_<?php echo $TPL_VAR["display_key"]?>();
		},500);
	});
});
...
<?php if($TPL_K1==$TPL_VAR["sort"]){?>
							<a href="?sort=<?php echo $TPL_K1?><?php echo $TPL_VAR["sortUrlQuerystring"]?>" sort_key="<?php echo $TPL_K1?>"><b><?php echo $TPL_V1?></b></a>
<?php }else{?>
							<a href="?sort=<?php echo $TPL_K1?><?php echo $TPL_VAR["sortUrlQuerystring"]?>" sort_key="<?php echo $TPL_K1?>"><?php echo $TPL_V1?></a>
<?php }?>
<?php }?>
<?php }}?>
			</span>
...
<?php if($TPL_V3->kind=='price'){?>
						<li class="gli_goodsprice">
							<div class="goods_price" <?php echo $TPL_V3->name_css?>>
<?php if($TPL_V2["string_price"]){?>
								<?php echo $TPL_V2["string_price"]?>
