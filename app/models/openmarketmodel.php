...
*/
	...
	## 마켓 금액 조정 설정 데이터 추출
	public function get_linkage_goods_config($goods_seq, $option_type = ''){
		$addSql[]	= $goods_seq;
				...
				$sql		= "select opt.option_seq, 
								...
								from fm_goods_option as opt
									...
									left join fm_linkage_goods_price_tmp as lgp 
										on ( tmp_seq = ? and opt.option_title = lgp.option_title and 
	...
	public function save_goods_send_mall($goods_seq, $send_mall = array()){
		if	($goods_seq && is_array($send_mall) && count($send_mall) > 0){
			$malldata	= $this->get_linkage_mall();
	...
	## 오픈마켓 주문 수집
	public function exec_order_receive(){
		$linkage = $this->get_linkage_config();
		$malldata		= $this->get_linkage_mall();
	...
	public function chg_price_to_option($goods_seq){
		...
		// 자동 계산 방식만 재계산한다. ( 수동은 처리하지 않음 )
		if	($config[0]['cal_type'] == 'auto'){
	...
	public function request_send_export($export_code){
		...
		left join fm_goods_export_item b on a.export_code=b.export_code
		left join fm_order_item c on b.item_seq=c.item_seq
		where a.export_code=?",$export_code);
