...
class brandmodel extends CI_Model {
	...
	public function get_brand_view($category_codes=array(),$maxDepth=4,$division=''){
	...
	public function get_list($code,$arrWhere=array()) {
		...
		$query = "select * from `fm_brand` where $whereStr order by `position` asc, `left` asc";
	...
	public function get_next_brand(){
		...
		if(strlen($cate['category_code'])<4){
			for($i=0;$i<(4-strlen($cate['category_code']));$i++){
				$category .= "0";
			}
	...
	public function get_brand_group_for_goods($goods_seq)
	{
	...
	public function get_brand_recommend_display_seq($categoryCode){
		...
		if($categoryData['recommend_display_seq'] && $displayData){
			...
			$query = "update fm_brand set recommend_display_seq=? where category_code=?";
	...
	public function set_brand_recommend($categoryCode,$params){
		...
		$recommend_display_seq = $this->get_brand_recommend_display_seq($categoryCode);
				...
				foreach($query->result_array() as $childCategoryData){
					...
					if($categoryData['recommend_display_seq']){
						...
						if($child_recommend_display_seq){
							$this->db->query("update fm_brand set recommend_display_seq='{$child_recommend_display_seq}', update_date=now() where category_code = '{$childCategoryData['category_code']}'");
						}
									
					/* 미사용시 하위제거 */
					}else{
						if($childCategoryData['recommend_display_seq']){
							$this->db->query("update fm_brand set recommend_display_seq='', update_date=now() where category_code = '{$childCategoryData['category_code']}'");
						}
					}
	...
	public function getSortValue($categoryCode, $type){
		switch($type){
			case 'min':
				$query		= $this->db->query("select min(l.sort) sortVal from fm_goods_option o, fm_goods g, fm_brand_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? ", $categoryCode);
				$sortData	= $query->row_array();
			break;
			case 'max':
				$query		= $this->db->query("select max(l.sort) sortVal from fm_goods_option o, fm_goods g, fm_brand_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? ", $categoryCode);
				$sortData	= $query->row_array();
			break;
			case 'cnt':
				$query		= $this->db->query("select count(l.category_link_seq) sortVal from fm_goods_option o, fm_goods g, fm_brand_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? group by g.goods_seq", $categoryCode);
				$sortData	= $query->row_array();
			break;
		}
