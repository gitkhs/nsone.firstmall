...
class Goodsreview extends CI_Model {
	...
	*/
	public function data_list($sc, $func = null) {
		...
		if( !empty($sc['isorder_seq']) && empty($sc['noorder_seq']) ) {//구매자
			$sqlWhereClause .= " and ( order_seq != 0) ";
		}elseif( empty($sc['isorder_seq']) && !empty($sc['noorder_seq']) ) {//비구매
			$sqlWhereClause .= " and ( order_seq = 0 or order_seq is null ) ";
		}

		if( !empty($sc['mborder_seq']) && empty($sc['nomborder_seq'])) {//회원
			$sqlWhereClause .= " and ( mseq != 0) ";
		}elseif( !empty($sc['nomborder_seq']) && empty($sc['mborder_seq']) ) {//비회원
		...
		if( !empty($sc['best']) && $sc['best'] == 'checked' ) {//best 검색
