...
FB.init({	appId: plus_app_id,	status: true,	cookie: true,	xfbml: true,	oauth: true});
/**
...
echo '<body><div id="fb-root"></div>';
		...
		if( $_GET['ssid']!=$this->session->userdata('session_id')) {
		...
		if($this->session->userdata('fbuser')) {
			...
			$redirecturl = "http://".$return_url."/snsredirect/openerreload?no=".$no."&likecount=".$count['like_count']."&fbuser=".$this->session->userdata('fbuser')."&fb_action_id=".$facebookactionid."&like_seq=".$like_seq."&mode=".$mode."&returndeleteaction=".$returndeleteaction."&facebooklikeopen=".$facebooklikeopen;
				if(!$returnid['error']){
