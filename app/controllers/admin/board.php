...
class Board extends admin_base {
	...
	**/
	public function data_list()
	{
		...
		if ( SERVICE_CODE == 'P_STOR' ) {
			$sc['whereis']	.= ' and id not in ("goods_qna","goods_review","bulkorder") ';//기본
		}else{
			$sc['whereis']	.= ' and id not in ("store_review","store_reservation","store_gallery","mbqna","bulkorder") ';//기본
		}
	...
	**/
	public function all_data_list()
	{
		...
		foreach($this->boardmanagerlist as $boardar){$idx++;
			...
			if( defined('__SELLERADMIN__') === true && SERVICE_CODE == 'P_STOR' ) {
				...
				//오프라인매장 전용게시판 제외
				if ( $boardid == 'store_review' || $boardid == 'store_reservation' || $boardid == 'store_gallery' ) continue;
			}
	...
	public function board_lists($ndata, $data, $sc, & $noticeloop, & $loop)
	{
		if( BOARDID != 'faq' ) {//공지사항용
			...
			foreach($ndata['result'] as $ndatarow){$idx++;
				...
				if(isset($notice['seq'])) {
					...
					$notice['modifybtn'] = '<span class="btn small valign-middle"><input type="button"  name="boad_modify_btn" board_seq="'.$notice['seq'].'"  board_id="'.BOARDID.'" value="수정" /></span>';
					...
					$notice['deletebtn'] = '<span class="btn small valign-middle"><input type="button" name="boad_delete_btn" board_seq="'.$notice['seq'].'"  board_id="'.BOARDID.'" value="삭제" /></span>'; 
		...
		foreach($data['result'] as $datarow){$idx++;
			...
			$datarow['reply_title']		= ($datarow['re_contents'])?'<span class="blue" >답변완료</span>':'<span class="gray" >답변대기</span>';//상태

			if( !(BOARDID == 'notice' || BOARDID == 'faq') ){
				$datarow['emoneylay']	 =  getBoardEmoneybtn($datarow, $this->manager);//적립금
			}
			...
			if($datarow['display'] == 1 ){//삭제시
				...
				$datarow['modifybtn'] = '<span class="btn small valign-middle"><input type="button"  name="boad_modify_btn" board_seq="'.$datarow['seq'].'"  board_id="'.BOARDID.'" value="수정" /></span>';
				if( BOARDID == 'bulkorder' || BOARDID == 'mbqna' || BOARDID == 'goods_qna'  || BOARDID == 'gs_seller_qna' ) {
					$datarow['replaybtn'] = ($datarow['re_contents'])?'<span class="btn small valign-middle"><input type="button" name="boad_reply_btn" board_seq="'.$datarow['seq'].'"  board_id="'.BOARDID.'" value="답변수정" /></span>':'<span class="btn small valign-middle"><input type="button" name="boad_reply_btn" board_seq="'.$datarow['seq'].'"  board_id="'.BOARDID.'" value="답변등록" /></span>';//관리자만가능
				}else{
					if($this->manager['auth_reply_use'] == 'Y') $datarow['replaybtn'] = '<span class="btn small valign-middle"><input type="button" name="boad_reply_btn" board_seq="'.$datarow['seq'].'"  board_id="'.BOARDID.'" value="답변" /></span>';
				}
				$datarow['deletebtn'] = '<span class="btn small valign-middle"><input type="button" name="boad_delete_btn" board_seq="'.$datarow['seq'].'"  board_id="'.BOARDID.'" value="삭제" /></span>';
	...
	**/
	public function view()
	{
		...
		**/ 
		...
		$data['emoneyviewlay']	=  getBoardEmoneybtn($data, $this->manager, 'view');//적립금
		$data['emoneylay']			=  getBoardEmoneybtn($data, $this->manager, 'btn');// 적립금
		...
		$this->boardurl->userviewurl = 'http://'.$_SERVER['HTTP_HOST'].'/board/view?id='.BOARDID.'&seq='.$_GET['seq'];//사용자보기(sns)
	...
	**/
	public function write()
	{
		...
		if($this->manager['write_admin_type'] == 'IMG' ) {
			$this->manager['icon_admin_img']		= ($this->manager['write_admin_type'] == 'IMG' && $this->manager['icon_admin_img'] && is_file($this->Boardmanager->board_data_dir.$this->manager['id'].'/'.$this->manager['icon_admin_img']) ) ? $this->Boardmanager->board_data_src.$this->manager['id'].'/'.$this->manager['icon_admin_img'].'?'.time():$this->Boardmanager->board_icon_src.'icon_admin.gif';
			$this->manager['writetitle'] = '<img src="'.$this->manager['icon_admin_img'].'" id="icon_admin_img"  align="absmiddle" style="vertical-align:middle;"/>';
		}else{
			$this->manager['writetitle'] = $this->manager['write_admin'];
		}
		}
		...
		if( !(BOARDID == 'notice' || BOARDID == 'faq') ){
			$data['emoneyuse']	 =  getBoardEmoneybtn($data, $this->manager,'use');//적립금지급여부
		}
	...
	**/
	public function manager_write()
	{
		...
		if(isset($_GET['id'])) {//수정시
			...
			//사용여부 설정가능
			$useform['auth_read']		= (BOARDID == 'notice' || BOARDID == 'faq' || BOARDID == 'mbqna'  || BOARDID == 'goods_qna' || BOARDID == 'gs_seller_notice'  || BOARDID == 'gs_seller_qna' ) ? '':'Y';

			// 작성권한-쓰기
			$useform['auth_write']	= (BOARDID == 'notice' || BOARDID == 'faq' || BOARDID == 'mbqna'  || BOARDID == 'gs_seller_notice' ) ? '':'Y';

			// 작성권한-답글
			$useform['auth_reply']		= (BOARDID == 'notice' || BOARDID == 'faq' || BOARDID == 'mbqna'  || BOARDID == 'bulkorder' || BOARDID == 'goods_review' || BOARDID == 'gs_seller_notice'  || BOARDID == 'gs_seller_qna' || BOARDID == 'store_reservation' || BOARDID == 'store_gallery') ? '':'Y';

			// 작성권한-댓글BOARDID == 'notice' ||
			$useform['auth_cmt']		= (BOARDID == 'faq' || BOARDID == 'mbqna' || BOARDID == 'bulkorder'  || BOARDID == 'gs_seller_notice'  || BOARDID == 'gs_seller_qna' || BOARDID == 'store_review' || BOARDID == 'store_reservation' || BOARDID == 'store_gallery') ? '':'Y';
			
			//글평가@2014-08-11
			$useform['auth_recommend']		= (BOARDID == 'faq' || BOARDID == 'mbqna' || BOARDID == 'bulkorder'  || BOARDID == 'gs_seller_notice'  || BOARDID == 'gs_seller_qna') ? '':'Y';
			//댓글평가@2014-08-11
			$useform['auth_cmt_recommend']		= (BOARDID == 'faq' || BOARDID == 'mbqna' || BOARDID == 'bulkorder'  || BOARDID == 'gs_seller_notice'  || BOARDID == 'gs_seller_qna') ? '':'Y';

			$useform['display']['skin']['use'] = (BOARDID == 'notice' || BOARDID == 'faq' || BOARDID == 'mbqna'  || BOARDID == 'goods_qna' || BOARDID == 'goods_review' || BOARDID == 'bulkorder' || BOARDID == 'gs_seller_notice' || BOARDID == 'gs_seller_qna' || BOARDID == 'store_review' || BOARDID == 'store_reservation') ? false:true;
			...
			if ( BOARDID == 'notice' || BOARDID == 'faq' || BOARDID == 'store_gallery' ) {
				$useform['display']['auth_write']['title']		= '슈퍼관리자';
			...
			//기능설정
			$useform['display']['autowrite_use']	= ( BOARDID == 'notice' || BOARDID == 'faq'  || BOARDID == 'mbqna' || BOARDID == 'bulkorder' || BOARDID == 'store_gallery') ? false:true;
			...
			if ( BOARDID == 'goods_review' ) {
				...
				//추가 조건 있는지 확인
				$qry = "select count(*) as cnt, max(bulkorderform_seq) as maxid from fm_boardform  where boardid='".BOARDID."' ";
				$query = $this->db->query($qry);
			...
			if( BOARDID != 'bulkorder' &&  BOARDID != 'notice' &&  BOARDID != 'faq' &&  BOARDID != 'gs_seller_notice' ){
				...
				for($i=0;$i<count($sms_arr);$i++){
					...
					## 발송제한시간 안내
					if(strstr($v['name'],"write") && $sms_rest['board_toadmin'] == "checked"){
						$v['sms_rest_admin'] = $restriction_msg;
					}else{
						$v['sms_rest_admin'] = '';
					}
					if(strstr($v['name'],"reply") && $sms_rest['board_touser'] == "checked"){
						$v['sms_rest_user'] = $restriction_msg;
					}else{
						$v['sms_rest_user'] = '';
					}
	...
	**/
	private function authForm($tag, $boardid = null, $type = 'B' ){
		...
		}else if($type == 'A' && $tag['name'] != 'auth_read' ){
	 ...
	 * @param $tag[selected] : 선택정보
	**/
	private function BoardListShowForm($tag) {  
	...
	public function getCommentlists($parent) {
		...
		$paginlay =  pagingtag($sc['searchcount']	,$sc['perpage'],$returnurl, getLinkFilter('',array_keys($sc)),'cmtpage',' viewlinkurl="viewlinkurl" ' );
