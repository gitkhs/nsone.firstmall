...
class joincheck extends front_base {
	...
	switch($data['check_type']){
		case 'comment':
			$mdata['type_ment'] = "댓글을 입력하면";
		break;
		case 'stamp':
			$mdata['type_ment'] = "'출석체크 버튼'을 누르면";
		break;
		case 'login':
			$mdata['type_ment'] = "로그인 하면";
		break;		
	}
	...
	if($chskin=='comment_basic'||$chskin=='comment_simple'){
		...
		if($mode != 'mylog'){
			...
			if($chskin=='comment_basic'){
				...
				$data['tdate']= date('Y년 m월 d일',strtotime($tdate));
			}	
					
			if($tdate <= $sdate){
				$data['prev_day']= 	"<spen ".$a_style.">".str_replace('-','.',$prev)."</span>";	
			}else{		
			...
			if($tdate >= $edate || $next > $today ){
				$data['next_day']= 	"<spen ".$a_style.">".str_replace('-','.',$next)."</span>";
			}else{
	...
	//달력형
		...
		$data['tdate']= date('Y년 m월',strtotime($tdate));
