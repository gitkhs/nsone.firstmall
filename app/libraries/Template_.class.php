...
class Template_
{
	...
	function print_($fid, $scope='', $sub=0)
	{
		...
		if (!isset($this->tpl_[$fid])) {
			...
			$this->exit_('에러 ', '템플릿 아이디 <b>'.$fid.'</b>가 정의되지 않았습니다.');
		}
		$tpl=&$this->tpl_[$fid];
		if ($tpl[0]==='txt') {
	...
	function _get_compile_path($rpath)
	{
		...
		if (@!is_file($tpl_path)) {
			...
			// 최근본상품 메인으로 보냄(예외처리)
			if( preg_match('/recently.html/',$rpath) ){
				pageRedirect('../main/index', '', 'self');
				exit;
			}

			if( !preg_match('/admin/',$this->template_dir) ) 
				$errmsg = "<br/><br/>".$rpath.' 페이지가 존재하지 않습니다.<br/><br/>쇼핑몰 고객센터로 문의해 주십시오.';
			else
				$errmsg = "<br/><br/>".$rpath.' 페이지가 존재하지 않습니다.<br/><br/>퍼스트몰 고객센터(1544-3270)로 문의해 주십시오.';
			$this->exit_('Error #1',$errmsg);
	...
	function exit_($type, $msg)
	{
		if($type != 'Error #1'){
			...
			exit("<br />\n".'<div align="center" style="font:12px tahoma,arial;background:white">'.$msg."</div><br />\n");
		}
