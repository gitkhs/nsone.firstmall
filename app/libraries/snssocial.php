...
class Snssocial
{
	function __construct() {
		...
		$this->userauth		= ($this->ci->userauth)?$this->ci->userauth:'email,user_likes,publish_actions,user_birthday, read_friendlists';
		$this->adminauth	= ($this->ci->adminauth)?$this->ci->adminauth:'email,user_likes,manage_pages, publish_actions, user_birthday, read_friendlists';

		$this->snstype = array('sns_f', 'sns_t', 'sns_m', 'sns_n');//, 'sns_y' 2013-05-03 요즘서비스종료
		...
		if( $this->arrSns['key_f'] != '455616624457601' ) {//전용앱
			$this->__APP_VER__				= (isset($this->arrSns['fb_ver']))?$this->arrSns['fb_ver']:'1.0';//버전 기본앱 1.0, 전용앱 2014-04-30 이후 2.0 
		}else{
			$this->__APP_VER__ ='1.0';
		}
		if($_GET['facebook']=='Y' || $_GET['signed_request'] || $this->ci->session->userdata('fammercemode')){
			...
			if($_GET['signed_request']){
				...
				$this->ci->session->set_userdata('fammercemode', $_GET['facebook']);
			}else{
			...
			if ($this->fbuser) {
				try {
					...
					if(!$this->fbuser_profile) {
						$this->fbuser_profile = $this->facebook->api('/me','GET',array('access_token'=>$token)); // 유저 프로필을 가져 옵니다.
					}


					$this->signedrequest = $this->facebook->getSignedRequest();
					if($token){
						$fb_accounts = $this->facebook->api('/me/accounts','GET',array('access_token'=>$token));
					}else{
	...
	**/
		...
		**/
		function twitterloginurl($mtype, $mform = 'login', $facebooktype) {
			...
			if($mform == 'join') {
				$this->arrSns['callbackurl_t'] = 'http://'.($_SERVER['HTTP_HOST']).'/sns_process/twitterjoin?mtype='.$mtype.'&facebooktype='.$facebooktype;
			}else{
	...
	**/
		...
		**/
		function naverloginurl($mtype, $mform = 'login') {
			...
			## 접속한 도메인 확인. (calllbackurl 에서 www 제거 : 도메인, www 만 인정. 기타 서브도메인까지 맞추기 힘듬)
			if(!strstr("http",$arrSystem['domain'])) $arrSystem['domain'] = "http://".$arrSystem['domain'];
			$host_tmp	= parse_url($arrSystem['domain']);
			$domain		= $host_tmp['host'];
			$http_host	= str_replace("www.","",$host_tmp);

			if($domain){
				$this->arrSns['callbackurl_n'] = 'http://'.($domain).'/sns_process/naveruserck';
			}else{
				$this->arrSns['callbackurl_n'] = 'http://'.($arrSystem['subDomain']).'/sns_process/naveruserck';
			}

			$get_request_token_url = "https://nid.naver.com/oauth2.0/authorize";

			$this->client_id		= $this->arrSns['key_n'];
			$this->client_secret	= $this->arrSns['secret_n'];
		...
		**/
		function naveraccesstoken($mtype, $mform = 'login') {
			...
			if($_GET['error']){
				$this->ci->session->unset_userdata('naver_state');
				$this->ci->session->unset_userdata('mtype');
				...
				if( $sess_naver_state != $state ) {
					...
					if($res['error']){
						...
						if($data['result']['resultcode'] == '00'){ 
							...
							$nvuser['enc_id']	= $data['response']['enc_id'];
							...
							//회원정보 가져오기 실패하였습니다
							$this->ci->session->unset_userdata('naver_state');
							$this->ci->session->unset_userdata('naver_access_token');
							$this->ci->session->unset_userdata('mtype');
						}
					}
				}
		...
		**/
		function daumloginurl($mtype, $mform = 'login') {
			...
			if($domain){
				...
				$this->arrSns['callbackurl_d'] = 'http://'.($arrSystem['subDomain']).'/sns_process/daumuserck';
			}
		...
		**/
		function daumuserprofile($mtype, $mform = 'login') {
			...
			if($data['message'] == "OK"){ 
				...
				//회원정보 가져오기 실패하였습니다
				$this->ci->session->unset_userdata('daum_access_token');
				$this->ci->session->unset_userdata('mtype');
			}
	...
	**/
	...
	function facebookpermissions($facebook) {
		...
		if ($this->ci->session->userdata('fbuser')) {
			try{
				$fbpermissions =$facebook->api('/me/permissions',array('access_token'=>$this->ci->session->userdata('user_accesstoken'))); 
		...
		if($this->ci->session->userdata('fbuser')){
			if( $this->ci->__APP_NAMES__ ) {
				try{
					...
					$ret_obj = $this->facebook->api('/me/'.$this->ci->__APP_NAMES__ .':'.$actiontype .'', 'POST', array($this->ci->__APP_TYPE__ => $product_url,'access_token'=>$this->ci->session->userdata('user_accesstoken') ));
					//debug_var($ret_obj);
				}catch (FacebookApiException $o){
  ...
  function publishCustomActionLike($product_url)
	{
		...
		if($this->ci->session->userdata('fbuser')){
			...
			try{
				$ret_obj = $this->facebook->api('/me/og.likes', 'POST', array('object' => $product_url,'access_token'=>$this->ci->session->userdata('user_accesstoken') ));
				//debug_var($ret_obj);
			}catch (FacebookApiException $o){
				//debug_var($o);
	...
	**/
	function facebook_searchfriends($usename)
	{
		...
		if(!$this->fbuser){
			...
			try{
				$ret_obj = $this->facebook->api('/me/friends','GET',array('access_token'=>$this->ci->session->userdata('user_accesstoken'), "fields" => "id, name, picture"));
			}catch (FacebookApiException $o){
				$ret_obj['message'] = $o.message;
				$ret_obj['error'] = true;
	...
	function facebook_mefeed($data)
	{
		...
		if(!$this->fbuser){
			...
			try{
				...
				$result = $this->facebook->api('/me/feed?access_token='.$this->ci->session->userdata('user_accesstoken'),'POST', $params);
			}catch (FacebookApiException $o){
				$result['message'] = $o.message;
				$result['error'] = true;
			}
