	...
	'set_editor' : function(textareaObj){
		...
		for(i=0; i<fdomain.length; i++){
			 if (((fdomain.charCodeAt(i) > 0x3130 && fdomain.charCodeAt(i) < 0x318F) || (fdomain.charCodeAt(i) >= 0xAC00 && fdomain.charCodeAt(i) <= 0xD7A3))) 
			{
				...
				kordomain_img = 'http://'+fsubdomain+'/common/editor_image?redomain='+encodeURI(fdomain);
				kordomain_file = 'http://'+fsubdomain+'#path/pages/trex/file.html?redomain='+encodeURI(fdomain);
				break;
			}
		}
		...
		$.ajax({
			'url' : "/app/javascript/plugin/editor/_editor.html?initializedId="+initializedId+'&board_id='+board_id+'&file_use='+file_use+'&videouse='+videouse+'&tinyMode='+tinyMode,
			'global' : false,
