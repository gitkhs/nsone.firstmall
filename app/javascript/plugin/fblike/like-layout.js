try{ 
	if(!eval('facebooklikeboxok')){
		...
		var title = "♥좋아요! <span class='desc'>♥좋아요하고 할인받자!</span>";
		...
		$(function(){
			...
			$(".fb-og-like").live("click",function(e){
				...
				if( $(this).attr("data-fblikeseq") > 0 ) {
					mode = 'unlike';
					url += '&mode='+mode;
					openDialogConfirmtitle(title,"♥좋아요를 취소하시겠습니까?",width,height,function(){facebookopendialog(url);},function(){return false});
				}else{
					url += '&mode='+mode;
					facebookopendialog( url);
				}
