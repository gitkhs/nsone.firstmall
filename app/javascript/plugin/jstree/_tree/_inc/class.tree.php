...
class _tree_struct {
	...
	function _remove($id) {
		...
		$this->jstreedb->query("" .
			...
			"WHERE category_code = '{$result[0]['category_code']}'"
		);
		...
		$this->jstreedb->query("DELETE a FROM `fm_category_link` a left join fm_category b on a.category_code=b.category_code where b.id is null;");
	...
	function _move($id, $ref_id, $position = 0, $is_copy = false) {
		...
		if($node !== false && $is_copy == false) {
			if($this->get_category_depth($ref_id)+$this->get_child_depth($id) > 4){
		...
		if($is_copy){
			...
			foreach($children as $data)
			{
				...
				$query = "update `".$this->table."` set category_code='$code',regist_date=now(),update_date=now() where id='".$data['id']."'";
				$this->jstreedb->query($query);
			}
...
class json_tree extends _tree_struct {
	...
	function move_node($data) {
		...
		$ori_category_code = $res['category_code'];
		...
		/* 이동 */
			...
			$this->change_sides_category_code($ori_category_code,$category_code);

			/* 하위카테고리도 동일한 처리 */
			$this->jstreedb->query("select * from `".$this->table."_link` where category_code like '{$ori_category_code}%' and category_code!='{$ori_category_code}'");
			foreach($this->jstreedb->get_all("assoc") as $row){
				$new_child_category_code = preg_replace("/^".$ori_category_code."/",$category_code,$row['category_code']);
				$this->jstreedb->query("update `".$this->table."` set category_code='{$new_child_category_code}' where category_code='{$row['category_code']}'");

				$this->change_sides_category_code($row['category_code'],$new_child_category_code);
			}

			/* 새로운 카테고리의 상위 카테고리에 대한 상품연결 생성 */
			if(strlen($category_code)>=8){
				$this->jstreedb->query("select * from `".$this->table."_link` where category_code like '{$category_code}'");
				$result = $this->jstreedb->get_all("assoc");
				foreach($result as $row){
					for($i=4;$i<strlen($category_code);$i+=4){
						$tmp = substr($category_code,0,$i);
						$this->jstreedb->query("select * from `".$this->table."_link` where goods_seq='{$row['goods_seq']}' and category_code='{$tmp}'");
						$this->jstreedb->nextr();
						$res = $this->jstreedb->get_row("assoc");
						if(!$res['category_code']){
							$sql = "insert into `".$this->table."_link` set goods_seq='{$row['goods_seq']}', category_code='{$tmp}', regist_date=now();";
							$this->jstreedb->query($sql);
						}
					}
				}
		...
		return "{ \"status\" : 1, \"id\" : ".$id.", \"category_code\" : \"".$category_code."\" }";
	}
	...
	function get_child_depth($id){
		...
		$query = $this->jstreedb->query("select max(level) as level from `".$this->table."` where category_code like '{$category_code}%'");
		$this->jstreedb->nextr();
		$res = $this->jstreedb->get_row("assoc");
	...
	function change_sides_category_code($ori,$new){
		$this->jstreedb->query("update `".$this->table."_link` set category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `".$this->table."_group` set category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `fm_coupon_issuecategory` set category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `fm_design_display` set auto_category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `fm_download_issuecategory` set category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `fm_event_choice` set category_code='{$new}' where category_code='{$ori}'");
		$this->jstreedb->query("update `fm_member_group_issuecategory` set category_code='{$new}' where category_code='{$ori}'");
	}
	...
	public function _get_child_cnt($category_code){
		$query = $this->jstreedb->query("select count(*) as cnt from `".$this->table."` where category_code like '{$category_code}%' and category_code!='{$category_code}'");
		$this->jstreedb->nextr();
		$res = $this->jstreedb->get_row("assoc");
		return $res['cnt'];
	}
