...
class _tree_struct {
	...
	function _remove($id) {
		...
		$query = "SELECT recommend_display_seq FROM `".$this->table."` WHERE id = '{$id}'";
		$this->jstreedb->query($query);
		$result = $this->jstreedb->get_all("assoc");

		// 카테고리 추천상품 디스플레이 삭제
		if($result[0]['recommend_display_seq']){
			$this->jstreedb->query("" .
				"DELETE FROM `fm_design_display` " .
				"WHERE display_seq = '{$result[0]['recommend_display_seq']}'"
			);
		}

		// 카테고리 링크 삭제
		$this->jstreedb->query("" .
			"DELETE FROM `".$this->table."_link` " .
			"WHERE location_code = '{$result[0]['location_code']}'"
		);
		...
		$this->jstreedb->query("DELETE a FROM `fm_location_link` a left join fm_location b on a.location_code=b.location_code where b.id is null;");
	...
	function _move($id, $ref_id, $position = 0, $is_copy = false) {
		...
		if($node !== false && $is_copy == false) {
			if($this->get_location_depth($ref_id)+$this->get_child_depth($id) > 4){
				$this->msg = '카테고리는 최대 4차까지 가능합니다.';
				return false;
			}
		}
		...
		if($node !== false) {
			...
			if($ldif>5) return false; // 4뎁스까지만 허용
			...
			$parentListConfig = mysql_fetch_assoc($res);

			$location_code = $this -> get_insert_code($ref_id);
			$sql[] = "" .
				"INSERT INTO `".$this->table."` (" .
					"`".$this->fields["parent_id"]."`, " .
					"`".$this->fields["position"]."`, " .
					...
					"`list_image_decorations`, " .
					"`list_info_settings`, " .
					"`location_code`,`regist_date`,`update_date` " .
					") " .
				"VALUES (" .
					$ref_id.", " .
					$position.", " .
		...
		if($is_copy){
			...
			foreach($children as $data)
			{
				...
				$query = "update `".$this->table."` set location_code='$code',regist_date=now(),update_date=now() where id='".$data['id']."'";
				$this->jstreedb->query($query);
			}
	...
	function get_next_code($parentCode){
		if( $parentCode ){
			...
			$query = "".
				"select max(location_code) max from `".$this->table."` ".
				"where location_code like '$parentCode%' and length(location_code)=$len";
		}else{
			$query = "".
				"SELECT max(location_code) max FROM `".$this->table."` WHERE length(location_code)=4";
			$len = 4;
		}
		$this->jstreedb->query($query);
		$this->jstreedb->nextr();
	...
	function move_code($ref_id,$ids){
		...
		$query = "SELECT location_code,id FROM `".$this->table."` WHERE id in (".implode(',',$ids).") order by location_code asc";
		$this->jstreedb->query($query);
		$result = $this->jstreedb->get_all("assoc");
		foreach($result as $k => $tmp)
		{
			$old = $tmp['location_code'];
			if($k == 0){
				$n = strlen($next) - 4;
			...
			$query = "update `".$this->table."` set location_code = '$code' where id = '".$tmp['id']."'";
			$this->jstreedb->query($query);
	...
	function get_insert_code($ref_id){
		$parentCode = "";
		if($ref_id){
			$query = "SELECT location_code FROM `".$this->table."` WHERE id ='$ref_id'";
			$this->jstreedb->query($query);
			$this->jstreedb->nextr();
			$tmp = $this->jstreedb->get_row("assoc");
			$parentCode = $tmp['location_code'];
		}
...
class json_tree extends _tree_struct {
	...
	function move_node($data) {
		...
		$this->jstreedb->query("select * from `".$this->table."` where id='{$data["id"]}'");
		$this->jstreedb->nextr();
		$res = $this->jstreedb->get_row("assoc");
		$ori_location_code = $res['location_code'];
		...
		if((int)$data["copy"] && count($this->add_fields)) {
			...
			$this->jstreedb->query("select * from `".$this->table."` where id='{$id}'");
			$this->jstreedb->nextr();
			$res = $this->jstreedb->get_row("assoc");
		...
		/* 이동 */
			...
			$this->change_sides_location_code($ori_location_code,$location_code);

			/* 하위카테고리도 동일한 처리 */
			$this->jstreedb->query("select * from `".$this->table."_link` where location_code like '{$ori_location_code}%' and location_code!='{$ori_location_code}'");
			foreach($this->jstreedb->get_all("assoc") as $row){
				$new_child_location_code = preg_replace("/^".$ori_location_code."/",$location_code,$row['location_code']);
				$this->jstreedb->query("update `".$this->table."` set location_code='{$new_child_location_code}' where location_code='{$row['location_code']}'");

				$this->change_sides_location_code($row['location_code'],$new_child_location_code);
			}

			/* 새로운 카테고리의 상위 카테고리에 대한 상품연결 생성 */
			if(strlen($location_code)>=8){
				$this->jstreedb->query("select * from `".$this->table."_link` where location_code like '{$location_code}'");
				$result = $this->jstreedb->get_all("assoc");
				foreach($result as $row){
					for($i=4;$i<strlen($location_code);$i+=4){
						$tmp = substr($location_code,0,$i);
						$this->jstreedb->query("select * from `".$this->table."_link` where goods_seq='{$row['goods_seq']}' and location_code='{$tmp}'");
						$this->jstreedb->nextr();
						$res = $this->jstreedb->get_row("assoc");
						if(!$res['location_code']){
							$sql = "insert into `".$this->table."_link` set goods_seq='{$row['goods_seq']}', location_code='{$tmp}', regist_date=now();";
							$this->jstreedb->query($sql);
						}
					}
				}
		...
		return "{ \"status\" : 1, \"id\" : ".$id.", \"location_code\" : \"".$location_code."\" }";
	}
	function get_location_depth($id){
	...
	function get_child_depth($id){
		...
		$query = $this->jstreedb->query("select max(level) as level from `".$this->table."` where location_code like '{$location_code}%'");
		$this->jstreedb->nextr();
		$res = $this->jstreedb->get_row("assoc");
	...
	function change_sides_location_code($ori,$new){
		$this->jstreedb->query("update `".$this->table."_link` set location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `".$this->table."_group` set location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `fm_coupon_issuelocation` set location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `fm_design_display` set auto_location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `fm_download_issuelocation` set location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `fm_event_choice` set location_code='{$new}' where location_code='{$ori}'");
		$this->jstreedb->query("update `fm_member_group_issuelocation` set location_code='{$new}' where location_code='{$ori}'");
	}
	...
	function get_children($data) {
		...
		foreach($tmp as $k => $v) {
			$result[] = array(
				...
				"location" => $v[$this->fields["location_code"]],
	...
	public function _get_child_cnt($location_code){
		$query = $this->jstreedb->query("select count(*) as cnt from `".$this->table."` where location_code like '{$location_code}%' and location_code!='{$location_code}'");
		$this->jstreedb->nextr();
		$res = $this->jstreedb->get_row("assoc");
		return $res['cnt'];
	}
