...
class _tree_struct {
	...
	function _remove($id) {
		...
		$this->jstreedb->query("DELETE a FROM `fm_brand_link` a left join fm_brand b on a.category_code=b.category_code where b.id is null;");
...
class json_tree extends _tree_struct {
	function __construct($table = "tree", $fields = array(), $add_fields = array("title" => "title", "type" => "type", "best"=>"best", "title_eng" => "title_eng")) {
	...
	function change_sides_category_code($ori,$new){
		...
		$this->jstreedb->query("update `fm_design_display` set auto_brand_code='{$new}' where category_code='{$ori}'");
	}
	...
	function get_children($data) {
		...
		foreach($tmp as $k => $v) {
			$category = array();
			$category["attr"] = array("id" => "node_".$k, "rel" => $v[$this->fields["type"]]);
			$title = $v[$this->fields["title"]];
			/*
			if($v[$this->fields["title_eng"]]) {
				$title = $title." (".$v[$this->fields["title_eng"]].")";
			}
			*/
			if($v[$this->fields["best"]] === "Y") {
				$title = $title." ★";
			}
			$category["data"] = $title;
			$category["category"] = $v[$this->fields["category_code"]];
			$category["state"] = ((int)$v[$this->fields["right"]] - (int)$v[$this->fields["left"]] > 1) ? "closed" : "";
