 ...
 */
	...
	function curation_menu(){
		...
		### 큐레이션 발송 구분
		$loop	= array(
					array('name'=>'personal_coupon'		,'title'=>'이번 주 만료될 할인 쿠폰 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 만료 쿠폰으로 구매 유도</span>'),
					array('name'=>'personal_cart'		,'title'=>'장바구니/위시리스트 상품 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 관심 상품으로 구매 유도</span>'),
					array('name'=>'personal_membership'	,'title'=>'회원 멤버쉽 등급 변경 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 등급 혜택으로 구매 유도</span>'),
					array('name'=>'personal_emoney'		,'title'=>'다음 달 소멸 예정 적립금 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 소멸 적립금으로 구매 유도</span>'),
					array('name'=>'personal_timesale'	,'title'=>'타임세일 상품 판매 종료 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 종료 상품으로 구매 유도</span>'),
					array('name'=>'personal_review'		,'title'=>'상품 리뷰 작성 유도 알림','etc'=>' <span style="color:#747474;font-size:11px;">→ 리뷰 혜택으로 작성 유도</span>')
				);
	...
	function mypage_url($inflow='',$param='',$mode='v'){
		...
		if($inflow && $param){
			...
			if($sns['shorturl_use'] == "Y" && strstr($param['inflow'],"sms")){
				$mypage_short_url	= urlencode($mypage_short_url."/personal_referer/access?param=".$param_tmp);
				$mypage_short_url2	= get_shortURL($mypage_short_url);
				## 짧은 URL 오류시 긴 URL로 대체
				if($mypage_short_url2 == "INVALID_APIKEY"){
...
## 이번주에 만료되는 쿠폰 안내
	function send_reserv_coupon($emode='', $smode='',$logview='n'){
		...
		if($remind_send){
			...
			$file_path			= "../../data/email/".$personal_mode.".html";
...
## 다음달에 소멸되는 적립금 안내
	function send_reserv_emoney($emode='', $smode='',$logview='n'){
		...
		if($remind_send){
			...
			## 다음달 소멸예정 적립금 보유 회원 리스트
			$param				= array();
			$param['startdt']	= date("Y-m-01 00:00:00", strtotime(date("Y-m-d").' +1 months'));
			$param['enddt']		= date("Y-m-t 23:59:59", strtotime(date("Y-m-d").' +1 months'));
				...
				foreach($member_list as $mdata){
					...
					if($sms_send_use == "Y" && $mdata['sms'] == "y" && $mdata['cellphone']){
						...
						if($email_send_use == "Y" && $mdata['mailing'] == "y" && $mdata['email']){
							...
							$personal_param['member_seq']			= $mdata['member_seq'];
							$personal_param['username']				= $mdata['user_name'];
							$personal_param['mileage_rest']			= number_format($mdata['mileage_rest']);
							...
							if($mdata['mileage_rest'] > 0){
								$mdata['mileage_rest'] = number_format($mdata['mileage_rest']);
							}
				...
				if($glist['group_seq'] > 0){
					...
					## 혜택 조건
						if($sale_detail["sale_use"] == "Y"){
							$loop["sale_use"]				= number_format($sale_detail["sale_limit_price"])."원 이상 구매";
						}else{
							$loop["sale_use"]				= "조건없음";
						}
					## 혜택 금액
						if($sale_detail["sale_price"]){
							$loop["sale_price"]		= " ".number_format($sale_detail["sale_price"]);

							if($sale_detail["sale_price_type"] == "WON"){
								$loop["sale_price"]	.= "원 할인";
							}else{
								$loop["sale_price"]	.= "% 할인";
							}
						}else{
					...
					## 추가 적립
						...
						if($sale_detail["point_use"] == "Y"){
							$loop["point_use"]				= number_format($sale_detail["point_limit_price"])."원 이상 구매";
						}else{
							$loop["point_use"]				= "조건없음";
						}

						if($sale_detail["point_price"]){
							$loop["point_price"]	= " ".number_format($sale_detail["point_price"]);

							if($sale_detail["point_price_type"] == "WON"){
								$loop["point_price"].= "원 적립";
							}else{
								$loop["point_price"].= "% 적립";
							}
						}else{
						...
						if($sale_detail["reserve_price"]){
							$loop["reserve_price"]			= number_format($sale_detail["reserve_price"]);
							if($sale_detail["reserve_price_type"] == "WON"){
								$loop["reserve_price"]		.= "원 적립";
							}else{
								$loop["reserve_price"]		.= "% 적립";
							}
						}else{
							$loop["reserve_price"] = "";
						}
						...
						//$loop["reserve_price"]	= ($loop["reserve_price"])? "적립금 ".$loop["reserve_price"]:"";
					...
					if($sms_send_use == "Y" && $mdata['sms'] == "y" && $mdata['cellphone']){
						...
						if($mdata['cart_cnt'] > 0){
							...
							foreach($cart['list'] as $key => $data){
								...
								if(trim($data['image']) && file_exists(ROOTPATH.$data['image'])){
						...
						if($mdata['wish_cnt'] > 0){
							...
							foreach($wish['record'] as $key => $goods){
								...
								if(trim($goods['image']) && file_exists(ROOTPATH.$goods['image'])){
								...
								$goods['price'] = number_format($goods['price']);
						...
						if($mdata['wish_cnt'] > 0){
							...
							foreach($wish['record'] as $key => $goods){
								...
								if($event){
									...
									// 등급혜택가격,정보 포함
									$goods['sale_price'] = $CI->goodsmodel->get_sale_price($goods['goods_seq'], $goods['price'], $goods['r_category'] , $goods['sale_seq']);
									$goods['price']		= number_format($goods['sale_price']);
	...
	function send_reserv_review($emode='', $smode='',$logview='n',$cron=''){
		...
		if($remind_send){
			...
			if($reserves['autoemoney_review']){
				$reserves_emoney = number_format($reserves['autoemoney_review']);
				...
				foreach($member_list as $mdata){
					...
					if($sms_send_use == "Y" && $mdata['sms'] == "y" && $mdata['cellphone']){
						...
						$sql	= "select 
									 ...
									 ge.export_code,
									 ge.status,
									 (select image from fm_goods_image where goods_seq=oi.goods_seq and image_type='thumbView') as image
								from 
									fm_order as o
