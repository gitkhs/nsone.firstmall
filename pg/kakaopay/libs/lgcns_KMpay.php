...
// ini_set("display_errors", 1);
    class kmpayFunc {
        ...
        public function kmpayFunc($LogDir) {
			...
			@chmod(ROOTPATH.$LogDir,0777);
			if(!is_dir(ROOTPATH.$LogDir)){
				@mkdir(ROOTPATH.$LogDir."/");
			}
			
    	    $this->LogPath = ROOTPATH.$LogDir."/";
    	}
    ...
    class JsonString {
        ...
        public function JsonString($LogDir) {
			...
			@chmod(ROOTPATH.$LogDir,0777);
			if(!is_dir(ROOTPATH.$LogDir)){
				@mkdir(ROOTPATH.$LogDir."/".date('Ymd'));
			}
			
    	    $this->LogPath = ROOTPATH.$LogDir."/".date('Ymd');
    	}
